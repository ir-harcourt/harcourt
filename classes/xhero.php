...
class hero_class extends database_class {
    ...
    function load($options=array()) {
        global $database;
        $date=(array_key_exists("date", $options)) ? $options['date'] : date("Y/m/d");
        $portal_id=(array_key_exists("portal_id", $options)) ? intval($options['portal_id']) : $_SESSION[['portal_id']];
        $industry_id=(array_key_exists("industry_id", $options)) ? intval($options['industry_id']) : $_SESSION[['industry_id']];
        $language_code=(array_key_exists("language_code", $options)) ? $options['language_code'] : $_SESSION[['language_code']];
        ...
        $query_where=array();
        $query_where[]=new query_where("portal_id", "in", array(0, $portal_id));
        $query_where[]=new query_where("industry_id", "in", array(0, $industry_id));
        $query_where[]=new query_where("language_code", "in", array("EN", $language_code));
        $query_where[]=new query_where("status", "in", $status);
        $query_where[]=new query_where("access", "in", $role_list);
        $query_where[]=new query_where("effective_date", "<=", $date);
        $query=array("select");
        $query[]="case (true)";
        if ($portal_id) $query[]="when portal_id={$portal_id} then 0";
        $query[]="when industry_id={$industry_id} then 1";
        $query[]="else 2";
        $query[]="end as sort_order,";
        $query[]="if (language_code = '{$language_code}', 0,1) as language_match,";
    ...
    function output($options=array()) {
        if ( (!$options['load']) && (!$this->load()) ) return;
        if ($options['session']) {
        ...
        $results=array();
        if ($options['preview']) $results[]="<p class=center>Visible content enclosed in <b><span class=scscpq_hero_preview>&nbsp;&nbsp;Box&nbsp;&nbsp;</span></b></p>";
