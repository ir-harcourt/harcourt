...
class portaloutput_class {
    ...
    function scs_table_version() {
        $results=array();
        $results['11/14/2025']="Initial release";
        return $results;
    }
    ...
    function load() {
        ...
        $this->query[]=implode("<br>", array($database->portal->meta->query, ""));
	    ...
	    $query_where[]=new query_where("content.record_id","=",$this->portal_id);
        ...
        $this->query[]=implode("<br>", array_merge($query, array("")));
	    $database->content->fetch(TRUE);
		$this->header=$database->content->data;
        $this->query[]=$database->content->meta->query;
    ...
    function image($options=array()) {
	    ...
	    $query[]="if (content.language_code='EN',2,1) as sort_key,";
        $query[]="content.* from content";
        $query[]="left join portalcontent on portalcontent.portal_id=" . fn_escape($this->portal_id) ." and portalcontent.form='Portal Image' \n and portalcontent.active=1";
        $query[]=$database->where($query_where);
        $query[]="order by sort_key,content.name";
	    $database->content->query($query);
        $this->query["image"]=implode("<br>", array_merge($query, array("")));
		$results=array();
    ...
    function headline() {
		...
		if (!$this->header->id) return;
		if ($this->header->headline) $results['headline']="<div class=portal_headline>{$this->header->description}</div>";
	    $query_where=array();
        ...
        $query[]="limit 1";
	    $database->content->query($query);
        $this->query["headline"]=implode("<br>", array_merge(array("<b>Headline</b>"),$query, array("")));
        while ($database->content->fetch = $database->content->fetch_array()) {
            $database->content->fetch();
            return $database->content->data->description;
        }
    }
