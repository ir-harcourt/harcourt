...
class user_toolbox_class {
	...
	function __construct($function, $role, $options=array()) {
		...
		if (!property_exists($this->options,"tabs")) $this->options->tabs=array();
...
/* Standard Login */
    ...
    function login_html() {
        ...
        $text=array("");
        $text[]="<style>";
        $text[]="table.scscpq_login { border-collapse: separate !important; width: 100% !important; font-size: 100% !important; }";
        $text[]="table.scscpq_login td { padding-top: .5em !important; padding-bottom: .5em !important; }";
        $text[]=".scscpq_login_left { width: 2em !important; vertical-align: top !important; }";
        $text[]=".scscpq_login_right { width: 99% !important; vertical-align: top !important; }";
        $text[]=".scscpq_login_error { font-weight: bold !important; }";
        $text[]=".scscpq_login_text_error { border: 1px solid red !important; }";
        $text[]="</style>";
        $text[]="";
        print implode("\n",$text);
	    print $forms->open();
		...
		$js[]="    login_action(jQuery('#action').val());";
		$js[]="})";
	    $js[]="function login_action(value) {";
	    $js[]="    switch (value) {";
	    $js[]="      case 'forgot':";
	    $js[]="       jQuery('#recaptcha_button').val('Email unlock code');";
	    $js[]="       break;";
	    $js[]="      case 'unlock':";
	    $js[]="       jQuery('#recaptcha_button').val('Enter unlock code');";
	    $js[]="       break;";
	    $js[]="      case 'new':";
	    $js[]="       jQuery('#recaptcha_button').val('Create new account');";
	    $js[]="       break;";
	    $js[]="     default:";
	    $js[]="       jQuery('#recaptcha_button').val('Login');";
	    $js[]="    }";
	    $js[]="}";
        $js[]="function login_checkbox(action) {";
        $js[]="    jQuery('#action_' + action).prop('checked',true);";
	    $js[]="    login_action(action);";
        $js[]="}";
	    $js[]="</script>";
        $js[]="";
        print implode("\n",$js);
        ...
        switch (TRUE) {
          ...
          case ($database->user->data->status != "Pending"):
           ...
           case ($database->user->data->cpqdomain_id):
            ...
            break;
          default:
            $text[]="Waiting for administrator apporval.";
        }
        if (sizeof($text)) {
            $text[]="";
            $results[]="<tr>";
...
EOT;
    ...
    function maintain_list() {
	    ...
	    $results[]="<table class='standard border'>";
        $results[]="<tr>";
        $results[]="<th colspan=2>Parameter Lookup</th>";
        ...
        $results[]="</tr>";
	    $results[]="</table>";
    	$results[]="<p>" . $forms->button("List",array("onclick"=>"toolbox_action('list','','');")) . "</p>";
...
EOT;
    ...
    function export() {
        global $database, $forms, $menu;
		$this->forms_title("User Export");
	    $forms->report['file_type']=$_POST['file_type'];
	    $forms->report['role']=array();
	    $menu->map=new map_class("user",$forms->report);
	    switch ($forms->report['action']) {
	      case "":
