...
class dashboard_toolbox_class {
    ...
    function html($options) {
        ...
        if ( ($_SERVER['REQUEST_METHOD']=="POST") && (strlen($_POST['action'])) ) {
            ...
            $name="{$action}_update";
            if (method_exists($this, $name)) $this->message=$this->$name($action, $message);
        }
	...
	function api_error($role, $subtype, $options=array()) {
	    ...
	    $results[]="<thead>";
	    $results[]="<tr>";
        if ($database->user->access($this->api_error->purge,FALSE)) $results[]="<th>Del?</th>";
	    $results[]="<th>SKU</th>";
	    $results[]="<th>Error</th>";
    ...
    function lookup_error($role, $subtype, $options=array()) {
	    ...
	    $results[]="<thead>";
	    $results[]="<tr>";
        if ($database->user->access($this->lookup_error->purge,FALSE)) $results[]="<th>Del?</th>";
	    $results[]="<th>SKU</th>";
	    $results[]="<th>Last Date</th>";
	    $results[]="<th>Count</th>";
	    $results[]="</tr>";
