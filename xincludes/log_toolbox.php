...
*/
    ...
    function list_entry() {
        ...
        $results[]="<tr>";
        $results[]="<td>&nbsp;</td>";
        $results[]="<td>" . $forms->button("Go!",array("onclick"=>"fn_page_item('list',0,0);")) . "</td>";
        $results[]="</tr>";
        $results[]="</table>";
        return $results;
    }
    function list_summary() {
        ...
        while ($database->log->fetch = $database->log->fetch_array() ) {
            $database->log->fetch();
            $results[]="<tr>";
            $results[]="<td class='center nobr'><a href=\"javascript:fn_page_item('item'," . $this->input->page . ",{$item});\">" . date("m/d/Y h:i A",$database->log->data->date_time) . "</a></td>";
            $text=array();
            switch (TRUE) {
        ...
        $results[]=$this->page_break->output($database->log->meta->found_rows);
		return $results;
    }
    function list_item() {
        global $database, $forms, $menu;
    ...
    function js() {
    	$results=array("");
        $results[]="<script>";
        $results[]="function toolbox_action(action, text) {";
        $results[]="if ( (text) && (!confirm(text)) ) return;";
