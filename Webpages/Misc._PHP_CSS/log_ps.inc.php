<?php
function log_ps2suburban( $action, $sku, $cad_code, $email, $ip ){

    include_once "/Webpages/Header_Footer/scs_header.php";
    include_once "classes/log.php";
    $database->connect();
  	$database->log->data=new log_data_class();
  	$database->log->data->ip=$ip;
  	$database->log->data->record_type=$action;
  	$database->log->data->sku=$sku;
  	$database->log->data->cad_code=$cad_code;
  	$database->log->data->email=$email;
