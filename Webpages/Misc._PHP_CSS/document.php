...
switch ($report['action']) {
  ...
  case "update":
    ...
    if ($database->document->data->parent_id && ($menu->errors_text['source'])) $menu->errors['source']=$menu->background['red'];
...
function url(page) {
	if (document.getElementById("url")) {
		document.getElementById("url").innerHTML=
        	"<b>&lt;a href='products.php?page=" +  page + "'&gt;" +
...
switch (TRUE) {
  ...
  case (($report['action'] == "update") && (sizeof($menu->errors))):
	print "<table class='standard noborder' width=1024>\n";
    if ($database->document->data->id) {
	    ...
	    print "<tr>\n";
	    print "<td>URL</td>\n";
	    print "<td><span id=url></span></td>\n";
	    print "</tr>\n";
    }
    print "<tr>\n";
    print "<td width=30%>Name</td>\n";
    print "<td width=70%><input type=text name=name id=name size=60 " . fn_html_value($database->document->data->name,"","",$menu->errors['name']) . " onchange=\"javascript:url('" . $database->document->data->id . "');\"></td>\n";
    print "</tr>\n";
    print "<tr>\n";
    print "<td>Description</td>\n";
	print "<td><textarea name=description rows=4 cols=60>" . $database->document->data->description . "</textarea></td>\n";
    print "</tr>\n";
    print "<tr>\n";
    ...
    print "url('" . $database->document->data->id . "');\n";
    print "</script>\n";
  ...
  default:
    ...
    print "<table class='border standard' width=1024>\n";
    print "<tr>\n";
    ...
    while ($database->document->fetch = mysql_fetch_array($database->document->meta->handle)) {
        ...
        if ($sort_key != $database->document->fetch['sort_key']) {
	        if ($sort_key) print "<tr><td colspan=6>&nbsp;</td></tr>\n";
	    ...
	    print "</tr>\n";
        if (!$database->document->data->parent_id) {
        	print "<tr><td>&nbsp;</td><td colspan=6><a href=\"javascript:fn_action(this,'maintain','0','" . $database->document->data->id . "','');\">Add new child</a></td></tr>\n";
		}
	}
	$database->document->free_result();
	print "<tr><td colspan=6>&nbsp;</td></tr>\n";
    print "<tr><td colspan=6><a href=\"javascript:fn_action(this,'maintain','0','0','');\">Add new parent</a></td></tr>\n";
    print "</table>\n";
}
print "</form>\n";
$database->disconnect();
