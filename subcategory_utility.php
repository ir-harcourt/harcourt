...
$forms->title("Subcategory Import/Export");

$forms->report['action']=$_POST['action'];
$forms->report['function']=$_POST['function'];
$forms->report['category_id']=$_POST['category_id'];
$forms->report['file_type']=$_POST['file_type'];
$forms->report['add']=FALSE;
if ($forms->report['category_id']) {
	$database->category->read($forms->report['category_id']);
    $forms->report['title']=$database->category->data->name;
  } else {
    $forms->report['title']="All records";
}

$map=new map_class("subcategory",$forms->report,$forms->report['category_id']);
...
// Upload/review functions begin
  ...
  case (($forms->report['function']=="upload") && ($forms->report['action']=="prelist")):
   ...
   case (($forms->report['function']=="upload") && ($forms->report['action']=="update")):
	...
	$forms->message[]=$map->import($options);
	break;
}
$menu->head();
print $forms->message();
...
switch (TRUE) {
  ...
  default:
	...
	print "<td>" . $database->category->select($forms->report['category_id']) . "</td>";
	print "</tr>";
