...
class scsps_toolbox_class {
    ...
    function json_utility_load() {
        ...
        $this->response->utility_configurator="<iframe id=scscpq_configurator_iframe src='{$url}' style='height: 1000px; width: 98%; frameborder: 0' onload='javascript:cad_listener(this.id);'></iframe>";
    }
...
/* Not implemented
	    ...
	    curl_setopt($this->curl_ch, CURLOPT_URL, "https://powertemp.scscpq.com/test.php");
	...
	//    curl_setopt($this->curl_ch, CURLOPT_POSTFIELDS, $this->curl_data);
        ...
        $this->curl_file=$_SERVER['DOCUMENT_ROOT'] . "/cad_curl.txt";
...
fn_pre($this->curl_url);
fn_pre($this->curl_info,1);
*/
        $this->response->curl_ipv4="curl -4 https://ip.hetzner.com";
    }
    ...
    function json_lookup_process() {
        ...
        if ($this->input->source == "sku") {
            $this->lookup=new cad_lookup_class($this->input->sku, $options, $cad_options);
            ...
            $url=$this->lookup->part_url;
          } else {
            switch ($this->input->source) {
              case "partid":
                $query=$database->cad->partid_prj($this->input->partid);
                break;
              case "prj":
                $query=$database->cad->prj_parse($this->input->prj);
                break;
              default:
                $query=array();
                $query['info']=$this->input->info;
                $query['varset']=$this->input->varset;
            }
            $options['prj']="info=" . $query['info']  . ((strlen($query['varset'])) ? "&varset=" . $query['varset'] : "");
            $url=$database->cad->url($options, $cad_options);
        }
    ...
    function html_lookup() {
        ...
        print "<td width='60%'>";
            ...
            print "<td>PRJ Info</td>";
            print "<td>" . $forms->textarea("info","",3,60,array("class"=>"input")) . "</td>";
            print "</tr>";
            print "<tr class=varset_class>";
            print "<td>PRJ Varset</td>";
            print "<td>" . $forms->textarea("varset","",8,60,array("class"=>"input")) . "</td>";
            print "</tr>";

            print "<tr class=partid_class>";
            print "<td>PartId</td>";
            print "<td>" . $forms->textarea("partid","",8,60,array("class"=>"input")) . "</td>";
            print "</tr>";
        ...
        print "<td width='40%'>";
            ...
            print "<td width='70%'>" . $forms->checkbox("staging", 1, preg_match("/" . preg_quote(".qa.") . "/i", $database->registry->cad->partserver_reversemap), array("class"=>"input")) . "</td>";
            print "</tr>";
            print "<tr>";
        ...
        print "</tr>";
        print "</table>";
    //    print "<div id=lookup_configurator></div>";
		print $forms->close();
		$menu->copyright();
    }
    function html_cad() {
    ...
    global $database, $forms;
		$this->cad_url=fn_url("https://www.partcommunity.com/cadqualifier.asp", array("firm"=>$database->registry->cad->catalog) );
		$xml=@simplexml_load_file($this->cad_url);
        $results=array();
		if ( (is_object($xml)) && (property_exists($xml,"format")) ) {
	        ...
	        foreach ($xml_data['format'] as $item) {
                ...
                $data->direct=( ($item['directintocad']) ? 0 : 1);
        ...
        switch (TRUE) {
          case (!is_object($xml)):
          	$this->cad_message="Cannot connect to URL";
            break;
          case (!sizeof($results)):
          	$this->cad_message="No records found";
            break;
          default:
          	$this->cad_message=number_format(sizeof($results)) . " records processed";
		}
        $comment=array();
