    ...
    function url($request=array()) {
        ...
        switch (TRUE) {
          case (strlen($request['info'])):
            $prj=$request['info'] . ( (strlen($request['varset'])) ? "&varset={$request['varset']}" : "");
            break;
        ...
        $url=fn_url(partcommunity_server($database->registry->cad->configurator, $request['staging']), $options);
        if (sizeof($hidePortlets)) $url .= "&" . implode("&", $hidePortlets);
        return $url;
    }
    function legacy_url($options=array(), $runtime_options=array()) {
        global $database;
		if (!array_key_exists("prj",$options)) $options['prj']="";
		if (!array_key_exists("sku_prj",$options)) $options['sku_prj']="";
		if (!array_key_exists("staging",$options)) $options['staging']=FALSE;
        $user_query=array();
        switch (TRUE) {
          case ($database->registry->cad->version == "3.0"):
          	break;
          case (array_key_exists("user",$options)):
			$user_query=$options['user'];
		    break;
          case ($_SESSION['user']->id):
			$user_query=$this->user($_SESSION['user']->id,$_SESSION['user']);
			break;
		}
        switch (TRUE) {
          case (strlen($options['info'])):
            $prj="info=" . $options['info'];
            $prj .= (strlen($options['varset'])) ? "&varset=" . $options['varset'] : "";
            break;
          case (strlen($options['sku_prj'])):
            $prj=$options['sku_prj'];
            break;
          default:
            $prj=$options['prj'];
        }
        $url=partcommunity_server($database->registry->cad->configurator, $options['staging']);
        if ( (!strlen($url)) || (!strlen($prj)) ) return;
        parse_str($prj, $test_query);
        $ok=TRUE;
        ...
        $url_query=(!$ok) ? $test_query : $url_query=array("prj"=>$prj);
        $url=fn_url($url,array_merge($url_query, $runtime_options, array(), $user_query));
        if ( (array_key_exists("hidePortlets",$options)) && (is_array($options['hidePortlets'])) ) {
            $text=array();
            foreach ($options['hidePortlets'] as $field) {
                $text[]="hidePortlets={$field}";
            }
            if (sizeof($text)) $url .= "&" . implode("&", $text);
        }
        return $url;
    }
...
class cad_lookup_class {
    ...
    var $cad_options;
/* Used by
C:\www\concentric\scscpq_api.php
C:\www\eaton\index.php
C:\www\eaton\scscpq_api.php
*/
    function __construct($sku, $options=array(), $cad_options=array()) {
        global $database;
		$this->sku=$sku;
        if (array_key_exists("hidePortlets", $options)) $this->hidePortlets=$options['hidePortlets'];
        ...
        $this->cad_options=(is_array($cad_options)) ? $cad_options : array();
    ...
    private function process() {
        ...
        $url_query=array();
	    $url_query['catalog']=$database->registry->cad->catalog;
	    $url_query['part']=$this->sku;
        $this->reversemap_url=fn_url(partcommunity_server($database->registry->cad->partserver_reversemap, $this->staging), $url_query);
        ...
        $url_query['catalog']=$database->registry->cad->catalog;
        $url_query['part']=$this->sku;
        $this->part_url=fn_url(partcommunity_server($database->registry->cad->configurator, $this->staging), array_merge($url_query, $this->cad_options));
        if (sizeof($this->hidePortlets)) {
            $text=array();
            foreach ($this->hidePortlets as $field) {
                $text[]="hidePortlets={$field}";
            }
            $this->part_url .= "&" . implode("&", $text);
        }
    }
}
