...
class WPMUDEV_Dashboard_Remote {
	...
	public function validate_hash( $hash, $id, $json ) {
		// Validation.
		if ( ! WPMUDEV_Dashboard::$api->has_key() || empty( $hash ) || empty( $id ) || empty( $json ) ) {
			return false;
		}
	 ...
	 * @since 5.0.0 Not validating nonce anymore, as its only timestamp based, which can be easily generated by an attacker.
	 * @since 5.0.1 Revised nonce validation to persist the latest accepted timestamp
	 *              and reject timestamps outside the allowed request-age and clock-skew window.
	 *
	 * @param string $id Request ID.
	 *
	 * @return bool
	...
	public function validate_nonce( $id ) {
		...
		// Extract the authenticated timestamp from the request ID.
		$id_parts = explode( '-', $id, 2 );

		if (
			2 !== count( $id_parts )
			|| '' === $id_parts[0]
			|| ! is_numeric( $id_parts[1] )
		) {
			return false;
		}
		...
		// Reject timestamps outside a three-minute window to tolerate clock skew and delivery delays.
		if (
			$timestamp < ( $current_time - ( 3 * MINUTE_IN_SECONDS ) )
			|| $timestamp > ( $current_time + ( 3 * MINUTE_IN_SECONDS ) )
		) {
			return false;
		}

		// Get the latest accepted request timestamp.
		$last_timestamp = (float) WPMUDEV_Dashboard::$settings->get(
			'hub_nonce',
			'general',
			0
		);
		...
		// Persist the latest accepted request timestamp.
...
// phpcs:disable Generic.CodeAnalysis.UnusedFunctionParameter.FoundAfterLastUsed
	...
	public function action_sync( $params, $action ) {
		...
		WPMUDEV_Dashboard::$site->refresh_local_projects( 'remote', $force );
	...
	public function action_delete( $params, $action ) {
		...
		if ( isset( $params->themes ) && is_array( $params->themes ) ) {
			foreach ( $params->themes as $theme ) {
				...
				$url = wp_nonce_url( 'themes.php?action=delete&stylesheet=' . rawurlencode( $slug ), 'delete-theme_' . $slug );
				ob_start();
	...
	public function action_replace_pro( $params, $action ) {
		...
		if ( ! empty( $project ) ) {
			...
			$success = WPMUDEV_Dashboard::$site->maybe_replace_pro_with_free( $project );
		}
	...
	protected function register_plugin_actions() {
		 ...
		 * Registers a Hub api action and callback for it
		 *
		 * @param array $actions_callbacks Array with action name as index, and callback as value.
		 */
	...
	protected function is_hub_request() {
		return WPMUDEV_Dashboard::$api->has_key() && ! empty( $_GET['wpmudev-hub'] ?? '' ); // phpcs:ignore
	}
	...
	protected function maybe_log_request() {
		if (
			( $this->is_hub_request() || ( function_exists( 'wp_doing_ajax' ) && wp_doing_ajax() && 'wpmudev_dashboard_admin_request' === ( $_POST['action'] ?? '' ) ) ) // phpcs:ignore
			&& defined( 'WPMUDEV_API_DEBUG' )
			&& WPMUDEV_API_DEBUG ) {
			...
			$msg = sprintf(
				$log,
				$_GET['wpmudev-hub'] ?? ( $_POST['action'] ?? '' ), // phpcs:ignore
				$this->current_action,
	...
	public function verify_signed_data( $data, string $signed_data ): bool {
		try {
			...
			$public_key_file = apply_filters( 'wpmudev_remote_verify_sign_pub_file_path', WPMUDEV_Dashboard::$site->plugin_path . 'keys/dashboard.pub' );
			...
			if ( defined( 'WPMUDEV_API_DEBUG' ) && WPMUDEV_API_DEBUG ) {
				error_log( // phpcs:ignore WordPress.PHP.DevelopmentFunctions.error_log_error_log
					'[WPMUDEV] Verify signed data error: ' . wp_json_encode(
						array(
