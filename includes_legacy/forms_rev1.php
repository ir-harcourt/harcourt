...
class forms_class {
    ...
    var $value_list=array("value","height","width","size","rows","cols","maxlength","frameborder","scrolling","title","align","border","target","pattern");
    ...
    function scs_version() {
    	$results=array();
        $results['12/25/2017']="Add async scripts";
        $results['05/25/2016']="Add www_js_body, www_js_footer";
        $results['03/21/2016']="Add readonly";
        $results['01/25/2016']="Add autocomplete option to open";
        $results['09/08/2015']=array("Add options to html_load","HTML5 default","Improve img function");
        $results['03/31/2015']=array("Enhance cmscontent");
        $results['02/24/2015']=array("Rewrite html class","Incorporate html5 characteristics");
        $results['01/12/2015']=array("Unlimited textarea size if cols=0","Add constant","Add functions dow, month");
        $results['09/16/2014']="Add span, div function";
        $results['07/30/2014']=array("Expand HTML options","Drop jquery_tab_error, jquery_tab");
        $results['06/25/2014']="Add html_open flag";
        $results['04/10/2014']=array("Add jquery_tab functions","Rewrite iframe");
        $results['01/10/2014']="Drop HTML5 number";
        $results['12/08/2013']="Add jquery datepicker";
        $results['11/27/2013']="Add P3P header for IE11";
        $results['10/24/2013']="Add HTML5 field types";
        $results['07/01/2013']="Add title_constant";
        $results['04/18/2013']="Add manual field type";
        $results['03/16/2013']=array("Enhance HTML headers","Resized image id replaced with code for security purposes");
        $results['02/25/2013']="Enhance escape function";
        $results['02/12/2013']="Add scs_tof bookmark";
        $results['01/21/2013']=array("Add CMS support","Include all named colors","Add registry_error");
        $results['12/24/2012']="Add font";
        $results['10/23/2012']=array("Integrate registry","Add html");
...
//http://www.w3schools.com/html/html_colornames.asp
    ...
    private function escape($text,$type="",$quotes=FALSE) {
		if (is_string($type)) list($type_code,$type_option)=explode(":",$type);
    ...
    function options($field,$options) {
		...
		$results=array("name"=>"","src"=>"","value"=>"","event"=>"","boolean"=>"","html5"=>"","manual"=>"");
	    foreach ($options as $key=> $text) {
			switch (TRUE) {
              case (!strlen($text)):
        ...
        if ($this->error[$field]) $output .= " {$this->error[$field]}";
		return $output;
    }
    function hidden($field,$text="",$text_type="") {
    ...
    global $database;
    	...
    	switch (TRUE) {
          case (in_array($type,array("password","email","number","url","tel"))):
          	$form_type=$type;
            break;
          default:
        	$form_type="text";
    ...
    function textarea($field,$data,$rows,$cols,$options=array()) {
		...
		return "<textarea" . $this->options($field,$options) . ">" . htmlspecialchars($text,ENT_HTML401) . "</textarea>\n"; 
    }
    ...
    function radio($field,$text,$compare,$options=array()) {
		if ($this->debug) scs_function_debug(__FILE__,__METHOD__,func_num_args(),func_get_args());
        if (!$options['id']) $options['id']="{$field}_{$text}";
		if (!isset($options['checked'])) $options['checked']=$this->compare($compare,$text,"checked");
		return "<input type=radio value='" . $this->escape($text) . "' " . trim($this->options($field,$options)) . ">\n";
    }
    function button($text,$options=array()) {
		if ($this->debug) scs_function_debug(__FILE__,__METHOD__,func_num_args(),func_get_args());
        if (!isset($options['class'])) $options['class']="nav_button";
        $options['value']=$this->escape($text);
		return "<input type=button " . trim($this->options($field,$options)) . ">\n";
    }
    function submit($text,$options=array()) {
		if ($this->debug) scs_function_debug(__FILE__,__METHOD__,func_num_args(),func_get_args());
        if (!$options['class']) $options['class']="nav_button";
        $options['value']=$this->escape($text);
		return "<input type=submit " . trim($this->options($field,$options)) . ">\n";
    }
    function iframe($field,$src,$options=array(),$resize=TRUE) {
        ...
        if (!array_key_exists('frameborder',$options)) $options['frameborder']=0;
        if (!array_key_exists('padding',$options)) $options['padding']="0px";
        if (!array_key_exists('scrolling',$options)) $options['scrolling']="auto";
        $forms_options=array();
        $style_options=array();
        foreach ($options as $key => $value) {
			if (in_array($key,array("height","width","frameborder","padding","scrolling"))) {
    ...
    function img($src,$options=array(),$img_options=array()) {
		...
		switch (TRUE) {
		  case (!$src):
          case (preg_match("/\$http:/i",$src)):
//		  case (sizeof($img_options)):  dropped 09/16/2015
			break;
		  default:
		...
		switch (TRUE) {
		  ...
		  case (array_key_exists('content',$img_options)):
            ...
            break;
		  case (array_key_exists('resize',$img_options)):
    ...
    function open($forms_options=array(),$options=array()) {
		...
		if ($forms_options['bookmark']) $forms_options['url'] .= "#" . $forms_options['bookmark'];
		$results=array();
		$results[]=trim($this->options($forms_options['name'],$options));
	    $results[]="method=" . $forms_options['method'];
	    $results[]="action='" . $forms_options['url'] . "'";
		if ($forms_options['target']) $results[]="target=" . $this->escape($forms_options['target']);
        $results[]="accept-charset='" . $this->html->charset . "'";
        if ($forms_options['data']) $results[]="enctype='multipart/form-data'";
    ...
    function title($title,$message="") {
		switch (TRUE) {
          case ($this->html->title):
			break;
		  case (isset($database->registry->www->title_prefix)):
			$this->html->meta("title",$database->registry->www->title_prefix . "  $title");
			break;
		  default:
			$this->html->meta("title",$title);
		}
        $this->message[0]=$this->title_constant;
        if ($message) {
        	$this->message[0] .= $message;
		  } else {
        	$this->message[0] .= $title;
		}
    }
    function dow() {
...
*/
    ...
    global $database;
		if (in_array($database->registry->cms->frame,array("alpha","numeric"))) $this->cms->meta();
        $this->html->open=TRUE;
        $headers=array();
        switch ($database->registry->www->http_cache) {
		  case "private":
	    ...
	    print "</head>\n";
	    print "<body";
        if ($options['body']) print " " . $options['body'];
        print ">\n";
    ...
    global $database;
		...
		if (in_array($database->registry->cms->frame,array("alpha","numeric"))) $this->cms->load();
...
class forms_html_class {
    ...
    global $database;
        ...
        $version=( (func_num_args() == 3) ? func_get_arg(2) : "");
		if ($type=="javascript") {
			if (!sizeof($this->javascript)) {
            	$this->javascript[]="<script>";
			  } else {
				array_pop($this->javascript);
			}
		...
		foreach ($data as $script) {
            ...
            switch (TRUE) {
			  ...
			  case ($type=="js"):
                ...
                if ( ($database->registry->www->js_async) && (preg_match("/^(http|https|ftp):/i",$script)) ) $content[]="async";
              ...
              case ($type=="async"):
				...
				$this->async[$script]="<script " . implode(" ",$content) . "></script>";
              	break;
              case ( ($database->registry->www->css_embed) && (!preg_match("/^(http|https|ftp):/i",$script)) ):
                ...
                break;
              case ( (!preg_match("/^(http|https|ftp):/i",$script)) && (strlen($version)) ):
				$this->css[$script]="<link rel='stylesheet' type='text/css' href='{$script}?version={$version}'>";
              	break;
			  default:
