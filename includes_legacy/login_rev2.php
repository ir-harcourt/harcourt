...
switch (TRUE) {
  case ($_POST['source']=="scs_menu"):
	...
	if ($database->registry->login->cookie) list($email,$forms->report['login_timestamp'])=explode("\t",$_COOKIE[$database->registry->login->cookie]);
  ...
  case ($_SERVER['REQUEST_METHOD'] == "GET"):
	...
	if ($_COOKIE[$database->registry->login->cookie]) {
	    list($forms->report['email'],$forms->report['login_timestamp'])=explode("\t",$_COOKIE[$database->registry->login->cookie]);
...
if ($forms->report['email']) {
	...
	$query=array("select");
    $query[]="password(" . fn_escape($forms->report['password']) . ") as 'login_password',";
...
switch (TRUE) {
  ...
  case ($forms->report['action']=="forgot"):
    switch (TRUE) {
      ...
      case ($database->user->data->status == "Forgotten"):
		$forms->error('email','',"Invalid Email Address (#F1L)");
    	break;
      case ($database->user->data->status == "Blacklist"):
		$forms->error('email','',"Invalid Email Address (#F2L)");
    	break;
      default:
  ...
  case ($forms->report['action'] == "unlock"):
    ...
    if ($forms->report['unlock_date'] == $database->user->data->login_timestamp) {
  ...
  case ($database->user->data->disabled);
    ...
    break;
  case (($database->user->data->auto_login) && ($database->user->data->login_timestamp == $forms->report['login_timestamp'])):
