...
$forms->message[]="Revised 02/22/2014";
$forms->report['action']=$_REQUEST['action'];
$forms->report['_login']=$_REQUEST['_login'];
list($forms->report['user_id'],$forms->report['last_login'],$forms->report['url'])=preg_split("/\t/",base64_decode($_REQUEST['_login']));
$database->user->read($forms->report['user_id']);
switch (TRUE) {
  case (!$database->user->meta->rows):
  case ($database->user->data->last_login != $forms->report['last_login']):
    die;
